...
class videoCoverSong extends StatefulWidget {
  const videoCoverSong({Key? key}) : super(key: key);

  @override
  State<videoCoverSong> createState() => _videoCoverSongState();
}

class _videoCoverSongState extends State<videoCoverSong> {
  ...
  load(){
    ...
    fileUploadList().uploadImageApi(type: 12.toString()).then((value) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Upload video cover Song", style: GoogleFonts.poppins(textStyle: appBarText)),
        // leading: InkWell(
        //     onTap: (){
      ...
      body: ListView(
        ...
        children: [
          ...
          /// add  video file
          const SizedBox(height: 10.0),
          Text(" Upload video cover song",style: GoogleFonts.poppins(textStyle: textLabel)),
          const SizedBox(height: 10.0),
          Container(
  ...
  Future<http.StreamedResponse> uploadMultiplePhotos({nextPage,file,location,captions,bandName,type,required BuildContext context}) async {
    ...
    try{
      if(response.statusCode==200){
        ...
        nextPage == "1" ?  Get.to(const videoCoverSong()) : Get.to(const originalSong());
        print(res.body);
      }else{
        print("Error");
        print(res.body);
